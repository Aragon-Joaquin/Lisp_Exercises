#|
Exercise 1.29: Simpson’s Rule is a more accurate method
of numerical integration than the method illustrated above.
Using Simpson’s Rule, the integral of a function f between
a and b is approximated as
h/3(y0 + 4y1 + 2y2 + 4y3 + 2y4 + ... + 2yn-2 + 4yn-1 + yn )
where h = (b - a)/n, for some even integer n, and yk =
f (a + kh). (Increasing n increases the accuracy of the ap-
proximation.) Define a procedure that takes as arguments
f , a, b, and n and returns the value of the integral, com-
puted using Simpson’s Rule. Use your procedure to inte-
grate cube between 0 and 1 (with n = 100 and n = 1000),
and compare the results to those of the integral procedure
shown above.
|#

;imports
(define (sum term a next b)
(if (> a b) 0 
  (+ (term a)
  (sum term (next a) next b)))
)


(define (cube c) (* c c c ))
(define (inc i) ( + i 1 ))


;definition
(define (simpson f a b n)
	(define (calculate-h) 
	  ( / (- b a) n)
	)

	(define (even_or_not k)
	  (if (or (= k 0) (= k n))
	    1 ; y
	    (+ 2 (* 2 (remainder k 2))) ; odd = 4y, even= 2y
	    )
	)
	  

	(define (term k)
	  (* (f (+ a (* k (calculate-h)))) (even_or_not k))
	)

	(define (calculate-y l) 
	  (sum term_sum l inc b)
	)

	

	( * 
	  (/ (calculate-h) 3)  
	  (sum term 0.0 inc n)   
	)
)

; i tried by doing it without help
; and i still dont get why the f arguments NEEDS to be cube
; i just understanded why it needed the cube... to calculate the area... in a integral... which calculates the area... OH MY

(print "simpson n=100: " (simpson cube 0 1 100))
(print "simpson n=1000: " (simpson cube 0 1 1000))
