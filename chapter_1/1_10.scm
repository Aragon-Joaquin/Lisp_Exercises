#|
The following procedure computes a math-
ematical function called Ackermann’s function.

(define (A x y)
    (cond ((= y 0) 0)
        ((= x 0) (* 2 y))
        ((= y 1) 2)
        (else (A (- x 1) (A x (- y 1))))))

What are the values of the following expressions?

(A 1 10)
(A 2 4)
(A 3 3)

Consider the following procedures, where A is the proce-
dure defined above:

(define (f n) (A 0 n))
(define (g n) (A 1 n))
(define (h n) (A 2 n))
(define (k n) (* 5 n n))

Give concise mathematical definitions for the functions com-
puted by the procedures f, g, and h for positive integer val-
ues of n. For example, (k n) computes 5n2
|#

(define (A x y)
    (cond ((= y 0) 0)
        ((= x 0) (* 2 y))
        ((= y 1) 2)
        (else (A (- x 1) (A x (- y 1))))))


(print (A 1 10))
; (A 0 (A 1 9 ))
; (A 0 (A 0 (A ( 1 8 )))) 
; (A 0 (A 0 (A 0 (A 1 7 ))))
; (A 0 (A 0 (A 0 (A 0 (A 1 6 )))))
; (A 0 (A 0 (A 0 (A 0 (A 0 (A 1 5 ))))))
; (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 1 4 ))))))) 
; (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 1 3 ))))))))
; (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 1 2 )))))))))
; (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 1 1 ))))))))))
; (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 2)))))))))
; (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 4))))))))
; (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 8)))))))
; (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 16))))))
; (A 0 (A 0 (A 0 (A 0 (A 0 32)))))
; (A 0 (A 0 (A 0 (A 0 64))))
; (A 0 (A 0 (A 0 128)))
; (A 0 (A 0 256))
; (A 0 512)
; 1024

(print (A 2 4))
; (A 1 (A 2 3))
; (A 1 (A 1 (A 2 2)))
; (A 1 (A 1 (A 1 (A 2 1))))
; (A 1 (A 1 (A 1 2)))
; (A 1 (A 1 (A 0 (A 1 1))))
; (A 1 (A 1 (A 0 2)))
; (A 1 (A 1 4))
; (A 1 (A 0 (A 1 3)))
; (A 1 (A 0 (A 0 (A 1 2))))
; (A 1 (A 0 (A 0 (A 0 (A 1 1)))))
; (A 1 (A 0 (A 0 (A 0 2))))
; (A 1 (A 0 (A 0 4)))
; (A 1 (A 0 8))
; (A 1 16) -> i refuse to do follow a +16 recursion stack of 2^16
; ...
; (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 128)))))))))
; ...
; (A 0 32768)
; 65536

(print (A 3 3))
; (A 2 (A 3 2))
; (A 2 (A 2 (A 3 1)))
; (A 2 (A 2 2))
; (A 2 (A 1 (A 2 1)))
; (A 2 (A 1 2))
; (A 2 (A 0 (A 1 1)))
; (A 2 (A 0 2))
; (A 2 4) <- BRO, ITS JUST 4^2 OF MAKING THE SAME CALCULATIONS FOLLOWING THE STACK
; ...
; (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 (A 0 128))))))))) <- same as previous one
; ...
; 65536


; ---------------------------------------------------

(define (f n) (A 0 n))
; f computes to 2 * n

(define (g n) (A 1 n))
; g computes to 2^n

(define (h n) (A 2 n))
; h computes to 2^(2^n)

; plot twist!!!

(print "\ntesting computing results")
(print (f 4)) ; should be 8 
(print (g 4)) ; should be 16
(print (h 4)) ; should be 65536
